...
export default function MancalaBoard({
  ...
  function drawPit(player: MancalaTurn, pit: Pit) {
    let clickable =
      game.getPit(player, pit) > 0 && game.getCurrentTurn() === player;
    let className = `pit player${(player as number) + 1}Board ${clickable ? " clickable" : ""}`;
    let onClick = clickable
      ? () => {
          onPitClick(pit);
        }
      : () => {};
  ...
  function drawHint(hint: Hint, player: MancalaTurn, hintIndex: number) {
    if (game.getCurrentTurn() !== player) {
      return (
        <div key={`hint-${player}-${hintIndex}`} className="hint disabled">
          <img
            className="pit-icon"
            src={Constants.IMG.FORBIDDEN_ICON}
            alt="Forbidden"
          />
        </div>
      );
    ...
    switch (hint?.hintType) {
      case Constants.HINT_TYPE.HIDDEN:
        return (
          <div className="hint disabled">
            <img
              className="pit-icon"
              src={Constants.IMG.HIDDEN_ICON}
              alt="Hidden"
            />
          </div>
        );
      case Constants.HINT_TYPE.NORMAL:
        return (
          <div key={`hint-${player}-${hintIndex}`} className="hint">
            {hint.value}
          </div>
        );
      case Constants.HINT_TYPE.UNKNOWN:
        return (
          <div key={`hint-${player}-${hintIndex}`} className="hint unknown">
            ?
          </div>
        );
      case Constants.HINT_TYPE.COMPLETED:
        return (
          <div key={`hint-${player}-${hintIndex}`} className="hint completed">
            {hint.value}
          </div>
        );
      case Constants.HINT_TYPE.INVALID:
        return (
          <div key={`hint-${player}-${hintIndex}`} className="hint disabled">
            <img
              className="pit-icon"
      ...
      default:
        return (
          <div key={`hint-${player}-${hintIndex}`} className="hint weird">
            ??
          </div>
        );
    }
  ...
  function drawPlayer1Hints() {
    ...
    for (let i = 5; i >= 0; i--) {
      hintElements.push(
        drawHint(hints[i], Constants.PLAYERS.PLAYER1 as MancalaTurn, i),
      );
    }
    return hintElements;
  }
  ...
  function drawPlayer2Hints() {
    ...
    for (let i = 0; i < 6; i++) {
      hintElements.push(
        drawHint(hints[i], Constants.PLAYERS.PLAYER2 as MancalaTurn, i),
      );
    }
    return hintElements;
  }
  ...
  return (
    <div className={`board-container ${isSimulating ? "board-disabled" : ""}`}>
      <div className="hint-grid">{drawPlayer2Hints()}</div>
      <div className="board">
        <div className="store player2Board">
          {game.getPit(Constants.PLAYERS.PLAYER2 as MancalaTurn, 0)}
        </div>
        <div className="pit-grid">
          {drawPlayer2Pits()}
          {drawPlayer1Pits()}
        </div>
        <div className="store player1Board">
          {game.getPit(Constants.PLAYERS.PLAYER1 as MancalaTurn, 0)}
        </div>
      </div>
      <div className="hint-grid">{drawPlayer1Hints()}</div>
    </div>
  );
}
